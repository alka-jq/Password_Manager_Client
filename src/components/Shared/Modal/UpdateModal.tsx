...
const UpdateModal = () => {
    const [editFolder, setEditFolder] = useState<{
        open: boolean;
        folder: { name: string; parent_id: number | null; folder_id?: number } | null;
        ...
        <>
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="w-1/3 bg-white lightmint:bg-[#629e7c] lightmint:text-white dark:bg-[#202127] text-[#000] font-medium rounded-2xl shadow-lg p-6 relative">
                    ...
                    >
                        <h1 className=" lightmint:text-white dark:text-white">Update folder</h1>


                        <div className="py-3 w-full">
                            <input
                                type="text"
                                // value={editFolder.folder.name}
                        ...
                        <div className="py-3 w-full">
                            ...
                            <div>
                                <select
                                    // value={editFolder.folder.parent_id || ''}
                                ...
                                >
                                    {/* <option value="">
                                        {selectedOption ? labels.find((el) => el.folder_id === selectedOption)?.name || '-- Select --' : '-- Select --'}
                                    </option> */}
                                    {/* {labels?.map((el) => (
                        ...
                        <div className="flex justify-end pt-3">
                            <button
                                type="button"
                                className="mr-3 lightmint:bg-green-50 lightmint:text-black dark:bg-[#2F2F2F] dark:border-[#2F2F2F] border-[1px] border-[#2565C7] px-7 py-2 rounded-lg hover:bg-[#bac1cc]  transition dark:text-white"
                            >
                                Cancel
                            </button>
                            <button
                                type="submit"
                                className="bg-[#2565C7] text-white lightmint:bg-green-50 lightmint:text-black dark:bg-[#2F2F2F] font-medium  px-7 py-2 rounded-lg dark:text-white"
                            >
                                Update
                            </button>
                        </div>
