...
const TrashList: React.FC = () => {
    ...
    const filteredData = data.filter(
        (item) => item.title.toLowerCase().includes(searchQuery) // 🔍 Search filter
    );
    ...
    const confirmPermanentDelete = async () => {
        try {
            ...
            setSelected((prevSelected) => prevSelected.filter((_, index) => !deleteTargetIds.includes(data[index]?.id)));
            dispatch(fetchItemCount());
        } catch (error) {
    ...
    return (
        <div className="">
                ...
                <div className="overflow-hidden bg-white rounded-md shadow-sm">
                    ...
                    <div className="overflow-y-auto max-h-[86vh]">
                        {filteredData.map((item, index) => (
                            ...
                            >
                                <div className="w-[5%] flex justify-center">
                                    <input type="checkbox" checked={selected[index] || false} onChange={() => toggleSelect(index)} className="accent-blue-500 w-4 h-4 cursor-pointer" />
                                </div>
                                <div className="w-[20%] flex justify-center">
                                    <button onClick={() => togglePin(index)} className="hover:text-blue-500 transition-colors" title={pins[index] ? 'Unpin' : 'Pin'}>
                                        {pins[index] ? <LuPin color="#3b82f6" /> : <LuPinOff />}
                                    </button>
                                </div>
                                <div className="w-[30%] truncate font-medium text-gray-800">{item.title}</div>
                                <div className="w-[30%]">
                                    <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${typeStyles[item.type] || 'bg-gray-100 text-gray-600'}`}>{item.type}</span>
                                </div>
                                <div className="w-[15%] flex justify-end gap-3 pr-3">
                                    <button onClick={() => handleRestore(item.id)} title="Restore" className="hover:text-green-600 transition-colors">
                                        <MdOutlineRestore size={18} />
                                    </button>
                                    <button onClick={() => handlePermanentDelete(item.id)} title="Delete Permanently" className="hover:text-red-500 transition-colors">
                                        <LuTrash2 size={18} />
                                    </button>
                                </div>
                            </div>
            ...
            {/* Modal */}
            {showDeleteModal && (
                <PermanentDeleteConfirmationModal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} onConfirm={confirmPermanentDelete} itemCount={deleteTargetIds.length} />
            )}
        </div>
    );
};
